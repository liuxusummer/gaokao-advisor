...
const GD_SUBJECTS_URL_TEMPLATE = 'https://eea.gd.gov.cn/xkqz/{guobiaoCode}.html'
...
export const guangdongSubjectScraper: SubjectScraper = {
  ...
  async scrape(client: HttpClient, year: number, options?: { force?: boolean }) {
    ...
    for (let i = 0; i < colleges.length; i++) {
      ...
      const url = GD_SUBJECTS_URL_TEMPLATE.replace('{guobiaoCode}', guobiaoCode)

      try {
        const result = await client.fetch(url, {
          cacheKey: `gd_${guobiaoCode}.html`,
          forceRefresh: options?.force,
        })

        const parsed = parseGdSubjects(result.html, college.id, college.name, url)
        records.push(...parsed)
