...
export async function loadColleges(): Promise<College[]> {
  ...
  const res = await fetchPublic('/data/common/colleges.json')
...
export async function loadMajors(): Promise<Major[]> {
  ...
  const res = await fetchPublic('/data/common/majors/catalog.json')
...
export async function loadScores(provinceName: string, year: number): Promise<ScoreRecord[]> {
  const res = await fetchPublic(`/data/scores/${encodeURIComponent(provinceName)}/scores_${year}.json`)
...
export async function loadSubjects(provinceName: string, year: number): Promise<Map<string, SubjectRequirement>> {
  const res = await fetchPublic(`/data/subjects/${encodeURIComponent(provinceName)}/subjects_${year}.json`)
...
export async function loadRankTable(provinceName: string, year: number): Promise<RankTableEntry[]> {
  ...
  const res = await fetchPublic(`/data/scores/${encodeURIComponent(provinceName)}/rank_table_${year}.json`)
...
export async function probeRankTableYears(provinceName: string): Promise<number[]> {
  ...
  const results = await Promise.all(
    years.map(async (year) => {
      try {
        const response = await fetchPublic(`/data/scores/${encodeURIComponent(provinceName)}/rank_table_${year}.json`)
